...
fun CardTransferItem(details: HomeStaticData) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .padding(5.dp)
            .wrapContentHeight(), colors = CardDefaults.cardColors(containerColor = White)
    ) {
        Column {
            Row(modifier = Modifier.fillMaxSize()) {
                Icon(imageVector = details.image, contentDescription ="account_img" , modifier = Modifier.weight(1f) )
